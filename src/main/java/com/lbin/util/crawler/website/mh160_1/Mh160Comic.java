...
//漫画url实体
@Data
@Gecco(matchUrl = "https://www.mh160_1.co/kanmanhua/{project}/", pipelines = "comicConsolePipeline")
public class Mh160Comic implements Serializable, HtmlBean {
    private static final long serialVersionUID = 1L;

    static {
        try {
            Field field = Mh160Comic.class.getDeclaredField("coverimg");
            Image image = field.getAnnotation(Image.class);
            ConfigProperties.setAnnotationValue(image, "download", ConfigProperties.getDownload());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
